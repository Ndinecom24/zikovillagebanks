            ...
            <div class="z-page-header">
                <div class="d-flex align-items-center justify-content-between flex-wrap" style="gap: 1rem;">
                    ...
                    <div class="d-flex align-items-center" style="gap: 1rem;">
                        <div style="position: relative;">
                            ...
                            @else
                                ...
                                @endphp
                                <div
                                    style="width: 56px; height: 56px; border-radius: 50%; background: rgba(255,255,255,0.2); border: 3px solid rgba(255,255,255,0.3); display: flex; align-items: center; justify-content: center; font-size: 1.2rem; font-weight: 800; color: #fff;">
                                    {{ $initials }}
                                </div>
                            @endif
                            <label for="avatarUploadInput" title="Change photo"
                                ...
                                <input type="file" wire:model="avatarUpload" accept="image/*" id="avatarUploadInput"
                                    style="display:none;">
                            </label>
                        </div>
                        <div>
                            ...
                            <p style="margin: 0;">
                                <code
                                    style="background: rgba(255,255,255,0.15); padding: 0.15rem 0.5rem; border-radius: 4px;">{{ $user->staff_no ?? 'N/A' }}</code>
                                &nbsp;{{ $user->job_title ?? 'No job title' }} &bull;
                                {{ $user->directorate ?? 'No directorate' }}
                            </p>
                        </div>
                    </div>
                    {{-- Action Buttons --}}
                    <div class="d-flex align-items-center" style="gap: 0.5rem;">
                        @if (!$editing)
                            <button wire:click="toggleEdit" class="btn btn-sm"
                                style="background: rgba(255,255,255,0.15); color: #fff; border-radius: 8px; font-size: 0.82rem;">
                                <span wire:loading wire:target="toggleEdit"
                                    class="spinner-border spinner-border-sm mr-1" role="status"></span>
                                <i wire:loading.remove wire:target="toggleEdit" class="fas fa-edit mr-1"></i> Edit
                                Profile
                            </button>
                        @endif
                        <button wire:click="togglePasswordReset" class="btn btn-sm"
                            style="background: rgba(255,255,255,0.15); color: #fff; border-radius: 8px; font-size: 0.82rem;">
                            <i class="fas fa-key mr-1"></i> Reset Password
                        </button>
                        <a href="{{ route('user.index') }}" class="btn btn-sm"
                            style="background: rgba(255,255,255,0.15); color: #fff; border-radius: 8px; font-size: 0.82rem;">
                ...
                <div class="row mt-3">
                    ...
                    <div class="col-6 col-md-3">
                        <div style="background: rgba(255,255,255,0.12); border-radius: 8px; padding: 0.5rem 0.75rem;">
                            <div style="font-size: 1.15rem; font-weight: 800; color: #fbbf24;">{{ $userRoles->count() }}
                            </div>
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="card z-card mb-0" style="border-bottom-left-radius: 0; border-bottom-right-radius: 0;">
                <div class="card-body py-0 px-0">
                    <ul class="nav nav-tabs" style="border: none; padding: 0 1rem;">
                        <li class="nav-item">
                            <button wire:click="switchTab('profile')" type="button"
                                class="nav-link {{ $activeTab === 'profile' ? 'active' : '' }}"
                                style="border-radius: 8px 8px 0 0; font-size: 0.85rem; font-weight: 600; border: none; background: none; cursor: pointer;">
                                <i class="fas fa-id-card mr-1"></i> Profile
                            </button>
                        </li>
                        <li class="nav-item">
                            <button wire:click="switchTab('offices')" type="button"
                        ...
                        <li class="nav-item">
                            <button wire:click="switchTab('roles')" type="button"
                                class="nav-link {{ $activeTab === 'roles' ? 'active' : '' }}"
                                style="border-radius: 8px 8px 0 0; font-size: 0.85rem; font-weight: 600; border: none; background: none; cursor: pointer;">
                                <i class="fas fa-shield-alt mr-1"></i> Roles
                                <span
                                    class="badge badge-warning ml-1">{{ $userRoles->count() + $officeRoles->count() }}</span>
                            </button>
                        </li>
                    </ul>
                </div>
            ...
            @if ($activeTab === 'profile')
                ...
                <div wire:key="tab-profile" class="card z-card"
                    ...
                    <div class="card-header d-flex align-items-center justify-content-between">
                        ...
                        @if (!$editing)
                            <button wire:click="toggleEdit" class="btn btn-sm"
                                style="background: #f3f4f6; color: #374151; border-radius: 6px; font-size: 0.78rem;">
                        ...
                        @if ($editing)
                            <div style="padding: 1rem 1.25rem;">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="z-label">Full Name <span
                                ...
                                <div class="d-flex" style="gap: 0.5rem; margin-top: 0.25rem;">
                                    <button wire:click="saveProfile" class="btn btn-sm z-btn-primary"
                                        wire:loading.attr="disabled">
                                        <span wire:loading wire:target="saveProfile"
                                            class="spinner-border spinner-border-sm mr-1" role="status"></span>
                                        <i wire:loading.remove wire:target="saveProfile"
                                            class="fas fa-check mr-1"></i> Save Changes
                                    </button>
                                    <button wire:click="cancelEdit" class="btn btn-sm"
                                ...
                                <table class="table mb-0" style="font-size: 0.85rem;">
                                    <tbody>
                                        ...
                                        @foreach ($personalFields as $field)
                                            <tr>
                                                <td
                                                    style="padding: 0.6rem 1rem; font-weight: 600; color: #6b7280; width: 160px; border-top: none; border-bottom: 1px solid #f3f4f6;">
                                                    {{ $field['label'] }}</td>
                                                <td
                                                    style="padding: 0.6rem 1rem; color: #111827; font-weight: 500; border-top: none; border-bottom: 1px solid #f3f4f6;">
                                                    {{ $field['value'] ?? 'N/A' }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table mb-0" style="font-size: 0.85rem;">
                                <tbody>
                                    ...
                                    <tr>
                                        <td
                                            style="padding: 0.6rem 1rem; font-weight: 600; color: #6b7280; border-top: none; border-bottom: 1px solid #f3f4f6;">
                                            Total Logins</td>
                                        <td
                                            style="padding: 0.6rem 1rem; border-top: none; border-bottom: 1px solid #f3f4f6;">
                                            <span class="badge badge-light"
                                                style="font-size: 0.82rem;">{{ $user->total_login ?? 0 }}</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td
                                    ...
                                    <tr>
                                        ...
                                        <td
                                            style="padding: 0.6rem 1rem; color: #111827; font-weight: 500; border-top: none; border-bottom: 1px solid #f3f4f6;">
                                            {{ $user->created_at ? $user->created_at->format('M d, Y') : 'N/A' }}</td>
                                    </tr>
                                    <tr>
                                        <td
                                            style="padding: 0.6rem 1rem; font-weight: 600; color: #6b7280; border-top: none; border-bottom: 1px solid #f3f4f6;">
                                        ...
                                        <td
                                            style="padding: 0.6rem 1rem; color: #111827; font-weight: 500; border-top: none; border-bottom: 1px solid #f3f4f6;">
                                            {{ $user->updated_at ? $user->updated_at->diffForHumans() : 'N/A' }}</td>
                                    </tr>
                                    <tr>
                                        <td
                                            style="padding: 0.6rem 1rem; font-weight: 600; color: #6b7280; border-top: none;">
                                            Roles</td>
                                        <td style="padding: 0.6rem 1rem; border-top: none;">
                                            @forelse($userRoles as $role)
                                                <span class="badge"
                                                    style="background: #fef3c7; color: #92400e; font-size: 0.72rem; margin-right: 0.2rem;">
                        ...
                        @if ($showPasswordReset)
                            <div style="border-top: 2px solid #fde68a; padding: 1rem 1.25rem; background: #fffbeb;">
                                ...
                                <p
                                    style="font-size: 0.78rem; color: #92400e; margin: 0 0 0.65rem; background: #fff; border: 1px solid #fde68a; border-radius: 6px; padding: 0.4rem 0.6rem;">
                                    <i class="fas fa-exclamation-triangle mr-1"></i>
                                    Resetting for <strong>{{ $user->name }}</strong>. They must change it on next
                                    login.
                                </p>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group mb-2">
                                            <label class="z-label">New Password <span
                                                    class="text-danger">*</span></label>
                                            <input type="password" wire:model="newPassword"
                                                class="form-control form-control-sm z-input"
                                                placeholder="Enter new password" id="resetPwd">
                                            @error('newPassword')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                            <div class="pwd-req mt-2" id="pwdReqGrid">
                                                ...
                                                <span id="rr-match"><i class="bi bi-circle"></i> Passwords
                                                    match</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group mb-2">
                                            <label class="z-label">Confirm Password <span
                                                    class="text-danger">*</span></label>
                                            <input type="password" wire:model="newPasswordConfirmation"
                                                class="form-control form-control-sm z-input"
                                                placeholder="Confirm password" id="resetPwdConfirm">
                                ...
                                <div class="d-flex" style="gap: 0.5rem;">
                                    <button wire:click="resetUserPassword" class="btn btn-danger btn-sm"
                                        style="border-radius: 8px; font-weight: 600; font-size: 0.78rem;"
                                        wire:loading.attr="disabled">
                                        <span wire:loading wire:target="resetUserPassword"
                                            class="spinner-border spinner-border-sm mr-1" role="status"></span>
                                        <i wire:loading.remove wire:target="resetUserPassword"
                                            class="fas fa-key mr-1"></i> Reset
                                    </button>
                                    <button wire:click="togglePasswordReset" class="btn btn-sm"
                                        style="background: #fff; color: #374151; border-radius: 8px; font-size: 0.78rem; border: 1px solid #d1d5db;">
                                        Cancel
                                    </button>
                                </div>
                            </div>
            ...
            @if ($activeTab === 'offices')
                <div wire:key="tab-offices" class="card z-card"
                    style="border-top-left-radius: 0; border-top-right-radius: 0;">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h3 class="mb-0" style="font-size: 1rem; font-weight: 700;">
                            <i class="fas fa-building mr-1" style="color: var(--z-green)"></i> Assigned Offices
                        </h3>
                        <span class="badge badge-success" style="font-size: 0.78rem;">{{ $user->offices->count() }}
                            office{{ $user->offices->count() !== 1 ? 's' : '' }}</span>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0" style="font-size: 0.85rem;">
                                <thead style="background: #f8fafc;">
                                    <tr>
                                        <th
                                            style="padding: 0.65rem 1rem; font-weight: 700; color: #374151; border-bottom: 2px solid #e5e7eb;">
                                            #</th>
                                        <th
                                            style="padding: 0.65rem 1rem; font-weight: 700; color: #374151; border-bottom: 2px solid #e5e7eb;">
                                            Office Name</th>
                                        <th
                                            ...
                                            Role in Office</th>
                                        <th
                                            style="padding: 0.65rem 1rem; font-weight: 700; color: #374151; border-bottom: 2px solid #e5e7eb; text-align: center;">
                                            Office Roles</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($user->offices as $idx => $office)
                                        <tr>
                                            <td style="padding: 0.6rem 1rem; vertical-align: middle; color: #94a3b8;">
                                                {{ $idx + 1 }}</td>
                                            <td style="padding: 0.6rem 1rem; vertical-align: middle;">
                                                <a href="{{ route('office.show', $office->id) }}"
                                                    style="font-weight: 600; color: #1a2332; text-decoration: none;">
                                                    <i class="fas fa-building mr-1"
                                                        style="color: var(--z-green); font-size: 0.75rem;"></i>
                                            ...
                                            <td style="padding: 0.6rem 1rem; vertical-align: middle; color: #6b7280;">
                                                @if ($office->pivot->role_in_office)
                                                    <i class="fas fa-tag mr-1"
                                                        style="font-size: 0.65rem; color: #9ca3af;"></i>{{ $office->pivot->role_in_office }}
                                                @else
                                                    <span style="color: #d1d5db;">&mdash;</span>
                                                @endif
                                            </td>
                                            <td
                                                style="padding: 0.6rem 1rem; vertical-align: middle; text-align: center;">
                                                @if ($office->roles->count() > 0)
                                                    <span class="badge badge-light"
                                                        style="font-size: 0.72rem; cursor: help;"
                                                        title="{{ $office->roles->pluck('name')->join(', ') }}">
                                                        {{ $office->roles->count() }}
                                                        role{{ $office->roles->count() > 1 ? 's' : '' }}
                                                    </span>
                                                @else
                                                    <span style="color: #d1d5db;">&mdash;</span>
                                                @endif
            ...
            {{-- ══════════════════════════════════ --}}
            @if ($activeTab === 'roles')
                <div wire:key="tab-roles" class="card z-card"
                    style="border-top-left-radius: 0; border-top-right-radius: 0;">
                    <div class="card-body p-0">
                        ...
                        <div class="card-header d-flex align-items-center justify-content-between"
                            ...
                            <a href="{{ route('user-roles.index') }}" class="btn btn-sm z-btn-primary"
                            ...
                            @forelse($userRoles as $role)
                                <div class="d-inline-flex align-items-center mr-2 mb-2"
                                    ...
                                    <a href="{{ route('roles.show', $role->id) }}"
                                        style="color: #92400e; text-decoration: none;">{{ $role->name }}</a>
                                </div>
                            @empty
                        ...
                        <div class="card-header d-flex align-items-center justify-content-between"
                            ...
                            <span class="badge badge-primary"
                                style="font-size: 0.78rem;">{{ $officeRoles->count() }}</span>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover mb-0" style="font-size: 0.85rem;">
                                <thead style="background: #f8fafc;">
                                    ...
                                    @forelse($officeRoles as $idx => $oRole)
                                        <tr>
                                            <td style="padding: 0.6rem 1rem; vertical-align: middle; color: #94a3b8;">
                                                {{ $idx + 1 }}</td>
                                            <td style="padding: 0.6rem 1rem; vertical-align: middle;">
                                                <a href="{{ route('roles.show', $oRole->id) }}"
                                                    style="font-weight: 600; color: #1e40af; text-decoration: none;">
                                                    <i class="fas fa-shield-alt mr-1"
                                                        style="font-size: 0.7rem; color: #2563eb;"></i>{{ $oRole->name }}
                                                </a>
                                            </td>
                                            <td style="padding: 0.6rem 1rem; vertical-align: middle;">
                                                <a href="{{ route('office.show', $oRole->source_office_id) }}"
                                                    style="font-weight: 600; color: #1a2332; text-decoration: none;">
                                                    <i class="fas fa-building mr-1"
                                                        style="font-size: 0.7rem; color: var(--z-green);"></i>{{ $oRole->source_office }}
                                                </a>
                                            </td>
                                            <td
                                                style="padding: 0.6rem 1rem; vertical-align: middle; color: #6b7280; font-size: 0.78rem;">
                                                Office &rarr; Role
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center text-muted py-4">
                                                ...
                                                <p class="mt-2 mb-0" style="font-size: 0.85rem;">No office-based
                                                    roles.</p>
                                            </td>
                                        </tr>
                                    @endforelse
