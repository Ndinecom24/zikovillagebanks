...
<style>
    ...
    .us-modal {
        ...
        overflow: hidden;
        animation: us-slideUp 0.25s ease;
    }
    .us-modal-head {
        display: flex;
        align-items: center;
    ...
    <div style="display:{{ $editing ? 'flex' : 'none' }};" class="us-modal-overlay" onclick="if(event.target===this){$wire.call('cancelEdit')}">
        <div class="us-modal" style="max-width:640px;">
            <div class="us-modal-head">
                ...
                <button wire:click="cancelEdit" class="us-modal-close" title="Close">&times;</button>
            </div>
    ...
    <div style="display:{{ $showPasswordReset ? 'flex' : 'none' }};" class="us-modal-overlay" onclick="if(event.target===this){$wire.call('togglePasswordReset')}">
        <div class="us-modal">
            <div class="us-modal-head">
                ...
                <button wire:click="togglePasswordReset" class="us-modal-close" title="Close">&times;</button>
            </div>
            <div class="us-modal-body">
                <div class="us-pwd-warn">
                    <i class="fas fa-exclamation-triangle mr-1"></i>
                    Resetting password for <strong>{{ $user->name }}</strong>. They will be required to change it on next login.
                </div>
            ...
            <div class="us-modal-foot">
                <button wire:click="resetUserPassword" class="nd-btn nd-btn-danger" style="border:1px solid #fecaca;" wire:loading.attr="disabled">
                    ...
                    <i wire:loading.remove wire:target="resetUserPassword" class="fas fa-key"></i> Reset Password
                </button>
...
(function() {
    function checkReqs() {
        ...
        var confirmEl = document.getElementById('resetPwdConfirm');
        if (!pwd || !confirmEl) return;
        var p = pwd.value, c = confirmEl.value;
        var checks = {
    ...
    // Delegated input listener for password requirements
    document.addEventListener('input', function(e) {
        if (e.target && (e.target.id === 'resetPwd' || e.target.id === 'resetPwdConfirm')) {
            checkReqs();
        }
    });
        ...
        if (e.key === 'Escape') {
            ...
            overlays.forEach(function(ov) {
                if (ov.style.display === 'flex') {
                    ...
                    if (ov.querySelector('#resetPwd')) {
                        comp.call('togglePasswordReset');
                    } else {
                        comp.call('cancelEdit');
                    }
                }
            });
        }
