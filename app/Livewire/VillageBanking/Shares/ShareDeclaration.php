...
class ShareDeclaration extends Component
{
    ...
    public function saveAllDeclarations()
    {
        ...
        // ── Compliance check: block non-compliant members ──
        $circle = Circle::with('villageBank')->find($this->circleId);
        if ($circle && $circle->villageBank) {
            $bankId = $circle->village_bank_id;
            $nonCompliant = [];

            foreach ($this->shares as $userId => $amount) {
                if (empty($amount) || !is_numeric($amount) || (float) $amount <= 0) continue;
                ...
                if ($membership && ! $membership->isCompliant()) {
                    ...
                    $nonCompliant[] = $user ? $user->name : "Member #{$userId}";
                }
            }

            if (! empty($nonCompliant)) {
                session()->flash('warning',
                    'The following members have not met compliance requirements (rules/constitution): '
                    . implode(', ', $nonCompliant)
                    . '. They must visit the Compliance Center before declarations can be recorded for them.'
                );
                return;
            }
        }
