...
class JoinRequestManager extends Component
{
    ...
    public function processAction()
    {
        ...
        if ($this->actionType === 'approve') {
            ...
            if ($memberRole) {
                $request->user->assignRole($memberRole);
            }

            // Send approval email + SMS notification
            $villageBank = VillageBank::find($request->village_bank_id);
            if ($villageBank && $request->user) {
                $request->user->notify(new MemberApprovedNotification($villageBank));
            }
            ...
            // Send rejection email + SMS notification
            $villageBank = VillageBank::find($request->village_bank_id);
            if ($villageBank && $request->user) {
                $request->user->notify(new MemberRejectedNotification($villageBank, $this->adminRemarks));
            }
